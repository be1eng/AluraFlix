...
const Modal = () => {
    const { isModalOpen, closeModal, selectedVideo } = useVideoContext();
    ...
    useEffect(() => {
        const fetchCategorias = async () => {
            try {
                ...
                if (videos) {
                    const uniqueCategorias = [...new Set(Object.values(videos)
                        .filter(video => video && video.categoria) 
                        .map(video => video.categoria)
                    )];
                    setCategorias(uniqueCategorias);
                } else {
                    setError('Error: The data fetched is not an array');
                }
            } catch (error) {
    ...
    const handleSave = async () => {
        try {
            if (editedVideo.id) {
                ...
                const response = await axios.post('https://aluraflix-ca481-default-rtdb.firebaseio.com/videos.json', editedVideo);
                const newVideo = { id: response.data.name, ...editedVideo };
